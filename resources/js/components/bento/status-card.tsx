...
const REGIONS = [
  { name: "VALIDATION", status: "ACTIVE", latency: "8.2ms" },
  { name: "ACTIVATION", status: "ACTIVE", latency: "7.8ms" },
  { name: "AUDIT_LOG", status: "ACTIVE", latency: "6.4ms" },
  { name: "COMPLIANCE", status: "ACTIVE", latency: "8.1ms" },
]
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between border-b-2 border-foreground px-4 py-2">
        ...
        <span className="text-[10px] tracking-widest text-muted-foreground">
          {`CHECK:${String(tick).padStart(4, "0")}`}
        </span>
      </div>
      <div className="flex-1 flex flex-col p-4 gap-0">
        ...
        <div className="grid grid-cols-3 gap-2 border-b border-border pb-2 mb-2">
          ...
          <span className="text-[9px] tracking-[0.15em] uppercase text-muted-foreground text-right">Response</span>
        </div>
        {REGIONS.map((region) => (
          <div
            key={region.name}
