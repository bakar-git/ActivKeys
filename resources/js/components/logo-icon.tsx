interface LogoIconProps {
    size?: number
    className?: string
}

/**
 * ActivKeys shield logo rendered as an inline SVG.
 * The two fill colours are driven by CSS custom properties so the icon
 * responds automatically to light / dark theme changes:
 *   - shield body  → var(--foreground)
 *   - key detail   → var(--background)
 */
export function LogoIcon({ size = 24, className }: LogoIconProps) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 512 512"
            width={size}
            height={size}
            className={className}
            aria-label="ActivKeys"
            role="img"
        >
            <path
                fill="var(--foreground)"
                d="m142.6 424.5l113.8 81.2 113.7-81.3c65.5-46.9 104.3-122.5 104.3-203l-0.1-121-218.2-93.4-218.1 93.6 0.1 121c0 80.5 38.9 156.1 104.5 202.9z"
            />
            <path
                fill="var(--background)"
                d="m365.2 100.3c-2.4 1.3-24.9 22.2-50.2 46.9-25.3 24.4-47.7 44.8-49.9 45.1-2.1 0.5-8.3-1.5-13.7-4.2-50.6-24.8-109.4-5.3-133.3 44.4-20.5 42.8-12.6 82 23.5 116.5 12.1 11.3 65.4 52 79.3 60.7 13.2 8 18.2 5.8 53.5-23.2 35.1-29.1 49.3-44.4 59-64.6 11.8-24.3 12.8-43.7 2.9-54.3-9.2-9.9-11-8.8-58.4 37.8-23.8 23.7-44.6 42.5-46.2 42.1-1.5-0.4-9.4-7.2-18-15.1-8.3-8.2-20.9-18.9-28.2-24.1-26.6-19-29.9-40.7-9.2-60.1 22.5-21.2 53.7-14.4 73.1 15.7 10.5 16.5 11.4 15.9 50-22.7 18.1-18.2 34.2-33 35.7-33 1.6 0 7 3.9 12 8.8 10.5 9.7 18.3 11.2 27.2 5.2 9.7-6.2 30.6-30.2 30.8-34.9 0-2.9-3.8-8.7-10.9-15.9l-10.8-11.4 8.8-9.3c10.4-11.3 12.7-19.8 8.1-30.9-7.2-17.4-23.3-26.3-35.1-19.5z"
            />
        </svg>
    )
}
